...
| BLE復旧snapshot | 非配付`scannerPoc`だけが実SDK adapterへ接続。releaseには未接続 | Auto Backupとdevice-to-device transferから除外する |
...
production/releaseへBLE adapterを接続していないため、releaseのアプリ側で宣言するのは次の範囲です。
...
依存ライブラリ由来のネットワーク権限は、アプリManifestの `tools:node="remove"` でrelease mergeから除外します。非配付`scannerPoc`だけは`BLUETOOTH_SCAN` / `BLUETOOTH_CONNECT`を要求し、legacy Bluetooth、位置情報、advertise、network権限を明示除去します。公式SDKは固定commitからローカル取得し、Gitやrelease artifactへ含めません。
...
## BLE PoCからproductionへ進む時の再審査

PoCのBLE adapterを配付対象へ変更する場合は、実装前に次を別変更として記録します。

1. Inateckの正式ライセンスと再配布条件、依存artifactのchecksumを確認する。
2. Android向けSDKのABI、target SDK、Manifest要求を実機評価で確認する。現行PoCは公式SDKを隔離・最小権限・ログ除去の条件で採用済みだが、通常releaseへの採用は再配付条件と実機受け入れが確定するまで行わない。
...
現時点では実機受け入れと配付条件の再審査が未完了なので、Android版のBLE機能完了やproduction採用は宣言しません。
