...
| `ProcessRecoveryInstrumentationTest` | `android/app/src/processRecoveryAndroidTest/java/jp/rimtty/codematch/ProcessRecoveryInstrumentationTest.kt`（専用propertyとhost runnerでのみ実行） |
...
| 4 | 音選択、音量、日英切替、再起動後 language 維持（`CodeMatchUITests.swift:202`） | `SettingsScreenTest.kt::scannerActionsAndSoundLanguageCallbacksAreEmitted`、`FeedbackContractTest.kt`、`AppFlowInstrumentationTest.kt::languageSelectionPersistsAcrossActivityRecreation`、`SettingsRepositoryTest.kt::languageSurvivesDataStoreReopen` + `::allSettingsSurviveIsolatedDataStoreReopen`、`AppLanguageSynchronizerTest` 6件、`AppLanguageSynchronizerInstrumentationTest.kt::frameworkAndAppLanguageRoundTripDoesNotCreateARecreationLoop`。加えて`ProcessRecoveryInstrumentationTest` + `run-process-recovery-tests.sh`が隔離emulatorアプリをOS force-stopし、新processの全設定値・英語UI・QR待機/Code 128待機/一致結果を検査する。Activity再生成、DataStore再オープン、実`LocaleManager`との双方向no-loopも検査済み。OS設定画面の実操作・実端末/OEMでの同じ復元操作は未検査 | P |
...
- Swift UI 5本のうち、Fake接続、一致→duplicate→読み直し→不一致、設定ガイド、実時間auto-advanceは同じdebug Fake/DI/repositoryを通すapp instrumentationへ昇格した。0件破棄、履歴名称変更・詳細・削除、履歴選択のActivity再生成/画面往復/system back、英語1/2 plural、テスト専用ランダムファイルを使ったDataStore/Room再オープン（active session、全checkpoint phase、全設定値）、PDF render/Intent契約も自動化済み。repository/Activity再生成テスト自体はOS process境界を含まない。追加の`ProcessRecoveryInstrumentationTest`は専用application IDで3段階を永続化し、hostが実アプリをforce-stopしてPID消失を確認した後、新processのMainActivityで工程・英語UIを検査する。一致結果のentry二重登録とcountdown再開も拒否する。API 37.1 emulatorの3ケース（seed/verify計6回）は成功し、CIはAPI 31/36でも同runnerを実行する。完了済み書込からの復元のみで、書込中断時の耐久性や実カメラ/BLEの復帰は検査していない。実DocumentProvider/共有先、実カメラ、対象BLEはPixel 7で部分的に受入済み、TalkBack/Switch Access、Samsung受け入れは打ち切り（[`STATUS.md`](STATUS.md)）。
