...
- `test-fixtures/matching-cases.json`: Swift/Kotlin共通の照合入力と期待結果。`schemaVersion` 2、47ケースで、各ケースはQRの仕向地を示す`destination`（`sawai` / `molten` / `denso`）を持つ。いずれの様式でもないQRを扱う2ケースだけが`destination`を持たない
