...
- The 2026-09-06 spec audit follow-ups landed: camera Code 128 gets the same `4-2-4@code` format validation as BLE on both platforms (#78), and `shared/test-fixtures/matching-cases.json` pins all 12 real label pairs plus same-series mix-ups as mismatches (#79). All 12 QR records are the real 66-char decodes of the label photos, plus two boxes of one part (`box-a` / `box-b`) that ground the duplicate rule. The photos themselves live in git-ignored `tmp/sawai-labels/` (customer data, never commit); decode them with a Vision `VNDetectBarcodesRequest` script when the label set changes.
