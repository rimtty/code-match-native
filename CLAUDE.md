...
- Open follow-ups from the 2026-09-06 spec audit: #78 (camera Code 128 must get the same `4-2-4@code` format validation BLE already has, both platforms) and #79 (extend `shared/test-fixtures/matching-cases.json` to all 12 real pairs; full 66-char QR payloads are known only for pairs #1 and #9).
...
- Code 128 from the camera requires the *same* value seen twice within 1.5s (`barcodeCandidate`) before it is accepted; only then does the camera stop and comparison run. A BCST-47 trigger is accepted once. BLE Code 128 is validated against the `4-2-4@code` business format (`TagBarcodeRecord.isValidScanPayload`); camera Code 128 is not yet (#78).
