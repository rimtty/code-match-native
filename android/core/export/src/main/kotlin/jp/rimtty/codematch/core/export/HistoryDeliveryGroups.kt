...
/**
 * The boxes of one Moltec delivery number.
 *
 * A Moltec slip repeats for every box of the same part, so the delivery number
 ...
 */
data class MoltecDeliveryGroup(
    ...
    val cumulativeQuantity: Int
        get() = entries.sumOf { it.moltecRecord()?.packQuantity ?: 0 }
}

/**
 * Groups entries by their Moltec delivery number in first-seen order.
 *
 * Only entries whose QR payload is a Moltec record contribute; a Sawai entry
 * and one saved without its payload are skipped, so a Sawai group returns an
 * empty list.
 */
fun List<MatchEntry>.moltecDeliveryGroups(): List<MoltecDeliveryGroup> {
    val records = LinkedHashMap<String, MoltecQrRecord>()
    val buckets = LinkedHashMap<String, MutableList<MatchEntry>>()
    forEach { entry ->
        val record = entry.moltecRecord() ?: return@forEach
...
/**
 * Formats a Moltec `MMDD` instruction date as `MM/DD`.
 ...
 */
fun formatMoltecDate(mmdd: String): String =
    if (FOUR_DIGITS.matches(mmdd)) "${mmdd.substring(0, 2)}/${mmdd.substring(2, 4)}" else mmdd

/**
 * Formats a Moltec `HHMM` instruction time as `HH:MM`, keeping null for the
 * blank field. A value that is not four digits is returned unchanged.
 */
fun formatMoltecTime(hhmm: String?): String? {
    val value = hhmm ?: return null
    return if (FOUR_DIGITS.matches(value)) "${value.substring(0, 2)}:${value.substring(2, 4)}" else value
}

/**
 * Parses an entry's QR payload as a Moltec record.
 * The destination is detected first so a Sawai payload can never be read at
 * Moltec field positions.
 */
internal fun MatchEntry.moltecRecord(): MoltecQrRecord? {
    val payload = qrPayload ?: return null
    if (CodeMatcher.detectDestination(payload) != Destination.MOLTEC) return null
    return MoltecQrRecord.parse(payload)
}
